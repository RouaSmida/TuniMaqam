...
{\Large A REST API Platform for Tunisian Maqam heritage (\textit{Tbu'a})\\[0.4cm]}
...
TuniMaqam follows a \textbf{layered architecture} with clear separation of concerns, adhering to REST principles: stateless requests, resource-based URLs, standard HTTP verbs (GET, POST, PUT, DELETE), and consistent JSON responses with appropriate status codes.
...
\node[draw, thick, minimum width=2.2cm, minimum height=1cm, align=center] (auth) {Auth};
\node[draw, thick, minimum width=2.2cm, minimum height=1cm, align=center, right=of auth] (knowledge) {Knowledge};
\node[draw, thick, minimum width=2.2cm, minimum height=1cm, align=center, right=of knowledge] (learning) {Learning};
\node[draw, thick, minimum width=2.2cm, minimum height=1cm, align=center, below=of auth] (analysis) {Analysis};
\node[draw, thick, minimum width=2.2cm, minimum height=1cm, align=center, below=of knowledge] (recommend) {Recommend};
\node[draw, thick, minimum width=2.2cm, minimum height=1cm, align=center, below=of learning] (external) {AssemblyAI};
\node[draw, thick, minimum width=7cm, minimum height=0.8cm, align=center, below=1.5cm of recommend] (db) {Database (SQLAlchemy)};
\draw[->, thick] (auth) -- (db);
...
\midrule
\texttt{Maqam} & Bilingual names, emotions, ajnas, regions, difficulty, rarity\\
\texttt{MaqamAudio} & Audio URLs, uploader, approval status\\
\texttt{Contribution} & Type, payload, status (pending/accepted/rejected)\\
\texttt{UserStat} & Scores, quiz counts, skill level, streak\\
\texttt{ActivityLog} & Activity type, points earned, timestamp\\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}[leftmargin=0.6cm, topsep=4pt, itemsep=2pt]
\item \textbf{Platform:} Render.com (PaaS) with auto-scaling
\item \textbf{Container:} Docker image built via GitHub Actions CI/CD
\item \textbf{Database:} Managed PostgreSQL with automated backups
\item \textbf{SSL:} Automatic HTTPS via Let's Encrypt
\item \textbf{Monitoring:} Health checks at \texttt{/ping}, logs via Render dashboard
\end{itemize}

\subsection{Security Implementation}

\begin{itemize}[leftmargin=0.6cm, topsep=4pt, itemsep=2pt]
\item \textbf{JWT Tokens:} HS256-signed, 24-hour expiry, contains user ID and role
\item \textbf{Rate Limiting:} Flask-Limiter with 200 requests/hour (auto-disabled in test mode)
\item \textbf{CORS:} Allowlisted origins only; preflight caching
\item \textbf{Audit Logging:} All mutations logged with user ID and timestamp
\item \textbf{Input Validation:} Marshmallow schemas reject malformed requests
\end{itemize}
...
The test suite comprises \textbf{85 passing tests} across 4 test files, executed via \texttt{pytest tests/ -v}:
...
\midrule
\texttt{test\_learning.py} & 22 & Quiz, flashcards, activities, leaderboard \\
\texttt{test\_auth.py} & 25 & JWT validation, OAuth, roles, protected routes \\
\texttt{test\_analysis.py} & 17 & Note analysis, confidence scoring, edge cases \\
\texttt{test\_recommendations.py} & 21 & Multi-factor ranking, heritage boost, context \\
\bottomrule
\end{tabular}
\end{center}
...
A 5-stage GitHub Actions pipeline (\texttt{.github/workflows/ci.yml}) automates quality assurance:
...
Pipeline triggers on push to \texttt{main} and all pull requests, ensuring code quality before merge.

\section{Cloud Hosting Configuration}

TuniMaqam deploys to Render.com via \texttt{render.yaml} Blueprint specification:

\begin{itemize}[leftmargin=0.6cm, topsep=4pt, itemsep=2pt]
\item \textbf{Web Service:} Python 3.11, Gunicorn with 4 workers
\item \textbf{Database:} PostgreSQL 15 with daily backups
\item \textbf{Environment:} Production secrets via Render dashboard
\item \textbf{Health Check:} \texttt{/ping} endpoint monitoring
\item \textbf{Auto-Deploy:} Git push triggers automatic deployment
\end{itemize}

\section{Docker Containerization}

Production deployment uses Docker (\texttt{Dockerfile} and \texttt{docker-compose.yml}):

\begin{itemize}[leftmargin=0.6cm, topsep=4pt, itemsep=2pt]
\item \textbf{Base Image:} Python 3.11-slim for minimal footprint
\item \textbf{Port:} 8000 (configurable via \texttt{PORT} env)
\item \textbf{Compose Services:} Web app + PostgreSQL database
\item \textbf{Volumes:} Persistent database storage, rotating logs
\end{itemize}
...
\begin{itemize}[leftmargin=0.6cm, topsep=4pt, itemsep=2pt]
\item \textbf{Swagger UI:} Interactive API explorer at \texttt{/apidocs}
\item \textbf{Authentication Guide:} \texttt{docs/AUTHENTICATION.md} - JWT flow, OAuth setup, role-based access
\item \textbf{Error Catalog:} \texttt{docs/ERROR\_CATALOG.md} - All error codes with causes and solutions
\end{itemize}

%------------------------------------------------------------------------------
\chapter{Declaration of AI/LLM Assistance}
...
\toprule
\textbf{Tool} & \textbf{Version} & \textbf{Primary Use}\\
\midrule
...
\bottomrule
\end{tabular}
\caption{LLM tools used during development}
\end{table}

\section{Areas Where LLM Assistance Was Beneficial}

\begin{enumerate}[leftmargin=0.8cm, topsep=4pt, itemsep=2pt]
\item \textbf{Boilerplate Generation:} Flask route decorators, SQLAlchemy model definitions, Marshmallow schemas
\item \textbf{Documentation:} API endpoint descriptions, docstrings, README content
\item \textbf{Code Patterns:} JWT authentication flow, error handling patterns, validation logic
\item \textbf{Testing:} pytest fixture generation, test case suggestions
\item \textbf{DevOps:} Docker configuration, GitHub Actions workflows, deployment scripts
\end{enumerate}

\section{Areas Where LLM Assistance Was Limited}

\begin{enumerate}[leftmargin=0.8cm, topsep=4pt, itemsep=2pt]
\item \textbf{Domain Knowledge:} Tunisian maqam theory required human expertise; LLMs lack deep understanding of regional musical traditions
\item \textbf{Algorithm Design:} The Precision-Coverage confidence model and multi-factor recommendation scoring required original mathematical formulation
\item \textbf{Cultural Context:} Heritage preservation priorities and UNESCO alignment decisions were human-driven
\item \textbf{Integration:} Connecting AssemblyAI transcription to musical note extraction required custom logic
\end{enumerate}

\section{Human Contributions}

The following aspects were entirely human-driven:

\begin{itemize}[leftmargin=0.6cm, topsep=4pt, itemsep=2pt]
\item Project vision, scope definition, and cultural preservation goals
\item Maqam database design (scale structures, regional mappings, historical context)
\item Selection and curation of authentic audio samples
\item Validation and testing of all generated code
\item Final architectural decisions and technology choices
\item User experience design for learning workflows
\item Security model design (role definitions, permission scopes)
\item All code review and quality assurance
\end{itemize}
